...
class SAMLAuthenticationProvider extends _base.BaseAuthenticationProvider {
  ...
  constructor(options, samlOptions) {
    ...
    _defineProperty(this, "maxRedirectURLSize", void 0);

    if (!samlOptions || !samlOptions.maxRedirectURLSize) {
  ...
  async login(request, attempt, state) {
    ...
    if (attempt.type === SAMLLogin.LoginInitiatedByUser) {
      const redirectURLPath = attempt.redirectURLPath || (state === null || state === void 0 ? void 0 : state.redirectURL);
    ...
    if (authenticationResult.succeeded() || authenticationResult.failed() && _tokens.Tokens.isAccessTokenExpiredError(authenticationResult.error)) {
      return await this.loginWithNewSAMLResponse(request, samlResponse, authenticationResult.state || state);
    }
  ...
  async logout(request, state) {
    this.logger.debug(`Trying to log user out via ${request.url.path}.`); // Normally when there is no active session in Kibana, `logout` method shouldn't do anything
    ...
    if (!(state === null || state === void 0 ? void 0 : state.accessToken) && !isIdPInitiatedSLO) {
    ...
    try {
      const redirect = isIdPInitiatedSLO ? await this.performIdPInitiatedSingleLogout(request, this.realm || (state === null || state === void 0 ? void 0 : state.realm)) : await this.performUserInitiatedSingleLogout(state === null || state === void 0 ? void 0 : state.accessToken, state === null || state === void 0 ? void 0 : state.refreshToken); // Having non-null `redirect` field within logout response means that IdP
  ...
  async loginWithSAMLResponse(request, samlResponse, state) {
    ...
    try {
      ...
      return _authentication_result.AuthenticationResult.redirectTo(stateRedirectURL || `${this.options.basePath.get(request)}/`, {
        state: {
          username,
          accessToken,
          refreshToken,
  ...
  async loginWithNewSAMLResponse(request, samlResponse, existingState) {
    ...
    const newState = payloadAuthenticationResult.state; // Now let's invalidate tokens from the existing session.
    ...
    } // At this stage both `existingState` and `newState` should already include `realm` and `username`
    // returned from saml/authenticate.


    if (newState.username !== existingState.username || newState.realm !== existingState.realm) {
      this.logger.debug('Login initiated by Identity Provider is for a different user than currently authenticated.');
      return _authentication_result.AuthenticationResult.redirectTo(`${this.options.basePath.serverBasePath}/security/overwritten_session`, {
        state: newState
      });
    }
  ...
  async performUserInitiatedSingleLogout(accessToken, refreshToken) {
    ...
    const {
      redirect
    } = await this.options.client.callAsInternalUser('shield.samlLogout', {
      body: {
        token: accessToken,
        refresh_token: refreshToken
      }
  ...
  async performIdPInitiatedSingleLogout(request, realm) {
    ...
    const {
      redirect
    } = await this.options.client.callAsInternalUser('shield.samlInvalidate', {
  ...
  captureRedirectURL(request, redirectURLPath = `${this.options.basePath.get(request)}${request.url.path}`, redirectURLFragment) {
    ...
    let redirectURLSize = new _configSchema.ByteSizeValue(Buffer.byteLength(redirectURLPath));

    if (this.maxRedirectURLSize.isLessThan(redirectURLSize)) {
      this.logger.warn(`Max URL path size should not exceed ${this.maxRedirectURLSize.toString()} but it was ${redirectURLSize.toString()}. URL is not captured.`);
    ...
    if (redirectURLFragment === undefined) {
      return _authentication_result.AuthenticationResult.redirectTo(`${this.options.basePath.serverBasePath}/internal/security/saml/capture-url-fragment`, {
        state: {
    ...
    redirectURLSize = new _configSchema.ByteSizeValue(Buffer.byteLength(redirectURL));

    if (this.maxRedirectURLSize.isLessThan(redirectURLSize)) {
      this.logger.warn(`Max URL size should not exceed ${this.maxRedirectURLSize.toString()} but it was ${redirectURLSize.toString()}. Only URL path is captured.`);
      redirectURL = redirectURLPath;
    } else {
